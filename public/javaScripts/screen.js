  ...
  //       document.getElementById("editReach").value = screen.reach;
  //       document.getElementById("editLatitude").value = screen.latitude || '';
  //       document.getElementById("editLongitude").value = screen.longitude || '';
    ...
    fetch(`/Dashboard/Screens/${screenid}`)
      ...
      .then((screen) => {
        const setValue = (id, value) => {
          ...
          if (element) {
            element.value = value || ''; // Set value or empty string
          } else {
            console.error(`Element with ID '${id}' not found`);
          }
        };

        setValue("editScreenid", screen.screenid);
        setValue("editPairingCode", screen.pairingcode);
        setValue("editScreenName", screen.screenname);
        setValue("editTags", screen.tags);
        setValue("editReach", screen.reach);
        setValue("editLatitude", screen.latitude);
        setValue("editLongitude", screen.longitude);
        setValue("editLocation", screen.location);
        setValue("editCity", screen.city);
        setValue("editState", screen.state);
        setValue("editCountry", screen.country);
        setValue("editPincode", screen.pincode);
        ...
        if (overlay) {
          ...
          console.error("Element with ID 'editOverlay' not found");
        }
      })
      .catch((error) => console.error("Error fetching screen:", error));
}
