...
const addScreen = async (req, res) => {
    ...
    console.log("in screem add function");
    
    // Check if the user has the 'Library delete' permission
    if (!user.permissions.includes('sc')) {
        console.log();
        
      req.flash('error_msg', 'You do not have permission to Add  Screen.');
      console.log('error_msg');
      // alert('error_msg', 'You do not have permission to delete media.')
      return res.redirect('/Dashboard/Screens'); // Redirect to library if permission is missing
    }
    ...
    try {
        ...
        const screenDeviceConfig=await screen.screenDeviceConfig();
            ...
            return res.render("Screen", {
                ...
                deletedScreens,
                groupscreen,
                screenStatus,screenDeviceConfig
            });
        }
...
const editScreen = async (req, res) => {
    ...
    // Check if the user has the 'Library delete' permission
    if (!user.permissions.includes('su')) {
    ...
    try {
        await screen.editScreen(
            ...
            state,
            country,
            pincode,reach,latitude,longitude
        );
...
const screenByPairingCode = async (req, res) => {
    const { pairingCode } = req.params;
    try {
        console.log("screen edit controller");
